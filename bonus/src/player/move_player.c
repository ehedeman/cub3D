...
/*   Updated: 2025/01/03 12:49:35 by ehedeman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	float cos_angle, t_map *map)
{
	if (player->key_up)
	{
		...
		else if (!touch(player->x, player->y + sin_angle * SPEED, map->map))
			player->y += sin_angle * SPEED;
		else if (!touch(player->x + cos_angle * SPEED, player->y, map->map))
			player->x += cos_angle * SPEED;
	}
	if (player->key_down)
	{
	...
	float cos_angle, t_map *map)
{
	if (player->key_left)
	{
		...
		else if (!touch(player->x + sin_angle * SPEED, player->y, map->map))
			player->x += sin_angle * SPEED;
		else if (!touch(player->x, player->y - cos_angle * SPEED, map->map))
			player->y -= cos_angle * SPEED;
	}
	if (player->key_right)
	{
